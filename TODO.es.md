...
**Idiomas / Languages:** [English](TODO.md) · **Español** (este archivo)

> Traducción de [TODO.md](TODO.md). Los dos archivos se mantienen en paralelo:
> si cierras o abres un ítem, Actualiza ambos, o la lista pierde su utilidad
> como estado del proyecto.
...
Documento de referencia del proyecto: [README.es.md](README.es.md) (o
[README.md](README.md) en inglés). Cada "Referencias:" de este documento apunta
al archivo del código del que habla, y las reglas que se aplican a este
repositorio están en [`.tdlearn/rules.toml`](.tdlearn/rules.toml).
...
- [x] README, licencia, empaquetado y CI reflejan el comportamiento real, en inglés y en español.
- [ ] Los releases se publican de forma reproducible (ver [CI-001](#p2--pruebas-rendimiento-y-entrega)).
...
- Referencias: [src/analysis/manifests.zig](src/analysis/manifests.zig), [src/analysis/resolver.zig](src/analysis/resolver.zig), [build.zig](build.zig) (`createModule`, `addTestStep`).
...
- Referencias: [src/analysis/manifests.zig](src/analysis/manifests.zig) (`readPackageAliasesAtRoot`), [src/analysis/graph_builder.zig](src/analysis/graph_builder.zig) (`buildImportEdgesAtRootWithContents`), [src/analysis/resolver.zig](src/analysis/resolver.zig) (`initWithAliases`).
...
- Referencias: [src/main.zig](src/main.zig) (`validateRoot`, `readFile`, `readOptionalFile`), [src/analysis/walker.zig](src/analysis/walker.zig) (`Walker.walk`, `Walker.appendFileNode`).
...
- Referencias: [src/main.zig](src/main.zig) (`parseOptions`, `printUsage`).
...
- Referencias: [src/core/toml.zig](src/core/toml.zig) (`parseValue`, `parse`), [src/core/rules.zig](src/core/rules.zig) (`parseRules`, `validateTomlSyntax`).
...
- Referencias: [src/analysis/walker.zig](src/analysis/walker.zig) (`walk`, `walkDir`, `flattenFiles`), [src/analysis/lang_registry.zig](src/analysis/lang_registry.zig), [src/main.zig](src/main.zig) (`filterSourcePaths`, `collectSourceNodes`).
...
- Referencias: [src/analysis/functions.zig](src/analysis/functions.zig) (`detectDecl`, `findBodyEnd`).
...
- Referencias: [src/analysis/imports.zig](src/analysis/imports.zig).
...
- Referencias: [src/analysis/resolver.zig](src/analysis/resolver.zig), [src/analysis/graph_builder.zig](src/analysis/graph_builder.zig).
...
- Referencias: [src/analysis/functions.zig](src/analysis/functions.zig), [src/analysis/classes.zig](src/analysis/classes.zig), [src/analysis/inherit_graph.zig](src/analysis/inherit_graph.zig).
...
- Referencias: [src/analysis/call_graph.zig](src/analysis/call_graph.zig) (`buildCallEdgesWithLimit`, `scanLine`).
...
- [x] Ignorar comentarios, strings y declaraciones inline correctamente (el scanner contaba como arista toda llamada dentro de un `//` o `/* */`).
...
- Referencias: [src/metrics/depth.zig](src/metrics/depth.zig), [src/metrics/mod.zig](src/metrics/mod.zig) (`computeHealth`).
...
- Referencias: [src/analysis/functions.zig](src/analysis/functions.zig) (`computeComplexity`), [src/core/types.zig](src/core/types.zig) (`FuncInfo`), [src/metrics/equality.zig](src/metrics/equality.zig) (`giniCoefficient`, `computeFunctionComplexityGini`).
...
- Referencias: [src/metrics/dead_code.zig](src/metrics/dead_code.zig) (`analyze`, `propagateReachability`, `collectDuplicateFlags`), [src/metrics/dead_code_test.zig](src/metrics/dead_code_test.zig).
...
- Referencias: [src/metrics/acyclicity.zig](src/metrics/acyclicity.zig), [src/metrics/modularity.zig](src/metrics/modularity.zig), [src/metrics/mod.zig](src/metrics/mod.zig) (`computeHealth`).
...
- Referencias: [src/metrics/depth.zig](src/metrics/depth.zig), [src/metrics/modularity.zig](src/metrics/modularity.zig), [src/metrics/dead_code.zig](src/metrics/dead_code.zig) (`isTestPath`).
...
- Referencias: [src/core/rules.zig](src/core/rules.zig) (`globMatch`, `validatePattern`, `checkRules`).
...
- Referencias: [src/core/rules.zig](src/core/rules.zig) (`Constraints.max_cyclomatic`/`max_cognitive`, `checkComplexityCeiling`, `ComplexityKind`), [src/main.zig](src/main.zig) (`Analysis.functions`, `JsonViolation`), [src/core/types.zig](src/core/types.zig) (`FileFuncs.lang`), [.tdlearn/rules.toml](.tdlearn/rules.toml).
...
- [x] Verificar que ambos techos fallan un `check` real (código 1) con una función deliberadamente compleja.
...
- Referencias: [src/core/path_utils.zig](src/core/path_utils.zig) (`canonicalRelative`, `isPackageIndexPath`), [src/analysis/walker.zig](src/analysis/walker.zig) (`normalizePaths`), [src/analysis/resolver.zig](src/analysis/resolver.zig) (`normalizeSeparators`).
...
- Referencias: [src/main.zig](src/main.zig) (`JsonScan`, `JsonCheck`, `JsonGate`, `makeJsonScan`, `printHumanScan`), [README.md](README.md) ("JSON Output").
...
- Referencias: [src/core/baseline.zig](src/core/baseline.zig) (`readBaseline`, `Baseline.validate`), [src/main.zig](src/main.zig) (`saveGate`, `compareGate`).
...
- Referencias: [src/core/settings.zig](src/core/settings.zig), [src/core/rules.zig](src/core/rules.zig) (`parseRules`).
...
- Referencias: [build.zig](build.zig) (`addTestStep`), [src/main.zig](src/main.zig) (`parseOptions`, `runScan`, `runCheck`, `runGate`).
...
- Referencias: [src/metrics/mod.zig](src/metrics/mod.zig) (`computeEquality`), [src/analysis/call_graph.zig](src/analysis/call_graph.zig) (`buildCallEdgesWithLimit`), [src/analysis/walker.zig](src/analysis/walker.zig) (`countLines`, `appendFileNode`).
...
- Referencias: [src/metrics/mod.zig](src/metrics/mod.zig), [src/metrics/dead_code.zig](src/metrics/dead_code.zig), [src/core/settings.zig](src/core/settings.zig), [src/core/types.zig](src/core/types.zig).
...
- Referencias: [src/metrics/dead_code.zig](src/metrics/dead_code.zig):739-806 (`collectDuplicateFlags`), [src/main.zig](src/main.zig) (salida de `scan`).
...
- Referencias: [.github/workflows/ci.yml](.github/workflows/ci.yml), [build.zig](build.zig) (`addRunCommand`).
...
- Referencias: [README.md](README.md), [build.zig.zon](build.zig.zon), `LICENSE`, `.gitattributes`.
...
- [x] Declarar `README.md`, `README.es.md`, `TODO.md`, `TODO.es.md` y `LICENSE` en `paths` de `build.zig.zon`.
- [x] Mantener cada documento en inglés y español en paralelo, con selector de idioma, enlaces entre documentos y enlaces desde cada documento al código que describe.
- [x] Mantener el código, los comentarios de código, los de `rules.toml` y los de CI en inglés, y decirlo en ambos README.
...
- Referencias: [.tdlearn/rules.toml](.tdlearn/rules.toml), [.tdlearn/baseline.json](.tdlearn/baseline.json).
...
- Referencias: [src/core/rules.zig](src/core/rules.zig) (1091 líneas), [src/core/rules_test.zig](src/core/rules_test.zig) (608), [src/main.zig](src/main.zig) (1152), [src/main_test.zig](src/main_test.zig) (383), [.tdlearn/rules.toml](.tdlearn/rules.toml) (`max_file_lines = 1200`), [README.md](README.md) ("This repository's own rules").
