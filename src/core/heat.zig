...
pub const HeatTracker = struct {
    ...
    const HeatEntry = struct {
        value: f64,
        last_modified: i64, // monotonic nanoseconds
    };
    ...
    pub fn init(allocator: Allocator, half_life: f64) !HeatTracker {
        return .{
            .entries = std.StringHashMap(HeatEntry).init(allocator),
            .trail = std.ArrayList(TrailEntry).init(allocator),
            .allocator = allocator,
            .half_life = half_life,
            .last_prune = 0,
        };
    ...
    pub fn get(self: *HeatTracker, path: []const u8) f64 {
        ...
        const now = std.time.timestamp();
        const elapsed = @as(f64, @floatFromInt(now - entry.last_modified));
        const elapsed_secs = elapsed / std.time.ns_per_s;
    ...
    pub fn pruneTrail(self: *HeatTracker, max_age_secs: f64) void {
        const now = std.time.timestamp();
        const cutoff = now - @as(i64, @intFromFloat(max_age_secs * @as(f64, @floatFromInt(std.time.ns_per_s))));
        var i: usize = 0;
...
test "HeatTracker basic record and get" {
    var heat = try HeatTracker.init(std.testing.allocator, 3.0);
    defer heat.deinit();
